...
export class PromptInjectionDetector {
  ...
  private readonly injectionPatterns: RegExp[] = [
    ...
    /disregard\s+(the\s+)?(above|previous|prior)\s+(instructions|guidelines|rules|settings|constraints|prompt|system)/i,
    /you\s+are\s+now\s+in\s+\w+\s+mode/i,
    /(switch|activate|enable)\s+(to\s+)?\w+\s+mode/i,
    /(admin|developer|debug|god|root|dan)\s+mode/i,
    /disregard\s+all\s+\w+\s+(measures|protocols|rules|guidelines)/i,
    /disregard\s+all\s+(rules|guidelines)(\s+and)?/i,
    /\b(enable|activate)\s+dan\b/i,
