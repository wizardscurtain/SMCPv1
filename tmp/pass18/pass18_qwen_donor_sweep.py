...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--out", default="/tmp/pass18.json"); ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu"); args = ap.parse_args()
    device = args.device
    ...
    result = {"protocol": "Pass18 Qwen1.5B causal affine-component donor sweep; behaviorally qualified by frozen Pass17", "model": MODEL, "revision": REVISION, "criteria": CRITERIA, "templates": {}, "overall_candidate_analogue": False}
    t0 = time.time()
