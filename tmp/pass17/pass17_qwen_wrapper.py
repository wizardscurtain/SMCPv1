...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--out", default="/tmp/pass17.json"); ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu"); args = ap.parse_args()
    out = Path(args.out)
    result = {"protocol": "Pass17 frozen Pass16 paraphrase-invariance Qwen scaling; no hidden-state inspection", "pass16_sha256": PASS16_SHA256, "models": {}, "selected_model": None}
    for mid in MODELS:
