...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--out",default="/tmp/pass19.json"); ap.add_argument("--device",default="cuda" if torch.cuda.is_available() else "cpu"); args=ap.parse_args()
    device=args.device
    ...
    result={"protocol":"Pass19 fixed-stage held-out prototype centroid and local-Jacobian test","model":MODEL,"revision":REVISION,"stage":STAGE,"block_index":BLOCK_INDEX,"train_assignments":TRAIN,"test_assignments":TEST,"criteria":CRITERIA,"templates":{},"overall_pass":False}
    t0=time.time()
    for tn,(names,tpl) in TEMPLATES.items():
        ...
        for vals in TRAIN+TEST:
            ...
            base_logits[vals]=torch.stack(ls)\n            if vals in TEST:\n                if any(COLORS[int(ls[q].argmax())] != vals[q] for q in range(3)): raise RuntimeError(f"baseline behavior changed under float32 eager backend: {tn} {vals}")
